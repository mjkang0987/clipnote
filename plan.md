...
### 설계
`app/_components/ClipsLoading.tsx`(서버 컴포넌트) 하나를 로케일별 `loading.tsx` 4개가
공유한다. 내용은 헤더 + 제목 + 로딩 문구 + `RunningDino`.
...
### 영향 파일
신규: `app/_components/ClipsLoading.tsx`, `app/clips/loading.tsx`,
`app/{en,ja,zh}/clips/loading.tsx`. 기존 파일 수정 없음.
...
### 미확인
**공룡이 실제로 도는지 눈으로 확인하지 못했다.** 자동화 브라우저 탭이 백그라운드라
(`visibilityState: "hidden"`) `requestAnimationFrame` 이 정지해 있어 이 환경에서는 어떤
경우에도 공룡이 움직이지 않는다. 코드 경로상 막힐 이유는 안 보이지만 실측이 아니다 —
로그인 상태로 `/clips` 를 열어 눈으로 확인해야 한다.
...
- 2026-08-04: 내 클립 로딩 화면(15장, #28) — `loading.tsx` 가 없어 서버가 인증·DB 를 끝낼
  때까지 이전 화면이 그대로 있었다. 로케일별 `loading.tsx` 4개가 공유하는 `ClipsLoading`
  추가(헤더·제목·로딩 문구·공룡). 3초 지연을 임시로 넣고 잰 결과 로딩 화면 도착이
  3.10s → 0.05s. 공룡이 실제로 도는지는 자동화 탭이 백그라운드(rAF 정지)라 미확인.
