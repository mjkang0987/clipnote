...
- **작업 분할 및 브랜치 생성:** 작업 요청 시 가장 작은 단위의 이슈로 나누고, **`main` 최신본을 기준으로**
  ...
  > `main`에서 따면 브랜치가 빌드하는 조합이 곧 배포될 조합이라 이 구멍이 닫힌다. (tas 저장소와 동일 규약)
...
- **Main 배포:** feature 브랜치에서 `main`으로 PR을 생성하고 머지를 **요청**할 것.
  ...
  > **왜 자동 머지를 없앴나** — 예전엔 `develop` 자동 머지였고 `develop`은 배포되지 않아 안전했다.
  > 이제 대상이 `main`인데 **`main` 푸시가 곧 Vercel 프로덕션 배포**이므로, 자동 머지는 무단 배포가 된다.
- **버전 펌핑:** 이 저장소는 release-please 가 Conventional Commits 로 버전을 자동 계산한다 →
  `package.json` 을 **수동으로 올리지 않는다**. 대신 커밋 접두사를 규모에 맞게 쓸 것.

## Work Request Flow (업무 처리 절차)
> 사용자가 업무를 요청하면 아래 순서를 따른다.

**세부 규약:**
- **이슈당 브랜치 · 이슈당 PR.** 브랜치명 `claude/issue-<번호>-<짧은슬러그>`, **`main`에서 분기 · `main`으로 PR**. `main`엔 직접 작업하지 않는다(항상 브랜치 경유).
- **PR 생성까지만 자동 진행.** 8단계(코드검증·CI)가 그린이면 PR을 열어 두고 보고한다. **`main` 머지는 지시자의 명시적 승인이 있을 때만** — 머지가 곧 배포다.
...
3. **작업** — **`origin/main` 최신본에서** 이슈당 브랜치를 만들어 구현. 커밋은 최소 단위·한국어·**Conventional Commits**(`On Commit` 준수).
4. **검증** — `/verify`로 빌드(`pnpm build`) + 실제 동작 확인.
5. **코드리뷰** — `/code-review`로 diff 리뷰.
   1. **리팩토링** — 지적 반영 + `/simplify`.
6. **재검증** — 리팩토링 후 다시 빌드·검증.
   1. **Main 재동기화(필수)** — `origin/main`을 병합하고 **4~6을 다시 통과**시킨다. 브랜치를 딴 뒤 `main`이 움직였으면 검증 기준이 낡은 것이다.
7. **PR 생성** — base는 **`main`**, 본문에 `Closes #<이슈>`. PR 생성 시 자동 CI(`.github/workflows/pr-review.yml`)가 실행된다.
8. **코드 검증** — PR 상태에서 최종 검증(`/verify`) + CI 결과 확인. 지적이 있으면 4~6 반복.
9. **머지** — 그린이고 **지시자 승인이 있으면** `main`으로 머지한다. 이슈 자동 종료, `index.md`·`plan.md` 갱신. **버전 범프는 하지 않는다**(release-please 자동).
10. **릴리스·배포** — `main`에 푸시되면 `release-please`가 릴리스 PR(버전·CHANGELOG)을 생성/갱신한다. 그 PR을 머지하면 `package.json` 버전·CHANGELOG·태그가 반영된다. **Vercel은 `main` 푸시마다 프로덕션 배포**하므로, 릴리스 PR 머지뿐 아니라 **기능 머지도 즉시 배포된다** — 그래서 9단계에 승인 게이트를 둔다.
