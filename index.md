...
- **게스트 클립 이전**: 구현됨. 로그인 시 로컬 클립이 있으면 계정으로 옮길지 **한 번 권한다**.
  거절해도 잃는 게 없다 — 목록 위 ‘이 브라우저에 남은 클립 n개 ›’ 진입 줄에서 전용 화면
  (`LocalClipsPanel`)으로 들어가 언제든 다시 옮기거나 모두 삭제할 수 있다. 계정 목록과는
  섞지 않는다(로컬 클립은 slug 가 없어 공유 링크를 만들 수 없다 — 섞으면 그 차이가 안 보인다).
...
- 2026-08-04: **로딩 화면의 공룡이 안 보이던 결함 수정.** `RunningDino` 가 `visibility: hidden` 으로 시작해 `useEffect` 로 자신을 보여 주는 구조인데 **Suspense fallback 은 하이드레이션되지 않아** 그 effect 가 영영 안 돌았다 — 바로 앞 항목에서 깐 로딩 화면의 공룡은 한 번도 화면에 나온 적이 없다(프로덕션 빌드에서 8초 내내 `computedVisibility: hidden` 확인). 기본 동작을 CSS 로 내려(`globals.css` 의 `.dino-sprite` — 바닥 왕복 + 4프레임) JS 없이도 보이고 걷게 하고, 하이드레이션된 화면에서는 컴포넌트가 그 애니메이션을 끄고 네 면 도는 동작을 이어받는다. 서버가 안 기다리는 화면(설정=인증 왕복, 로그인=OAuth 리다이렉트)은 클라이언트 대기 구간에 공룡을 붙였다. 상세는 `plan.md` 16장.
- 2026-08-04: 로딩 화면(`app/loading.tsx` + `ScreenLoading`). 서버가 인증·DB 를 끝낼 때까지 이전 화면이 그대로 있고 주소도 안 바뀌던 문제. 루트 하나가 하위 전 라우트를 덮고, 빨리 끝나는 화면에는 fallback 이 나가지 않는다. 로딩 화면 도착이 3.10s → 0.03s. 상세는 `plan.md` 15장. (공룡이 보이지 않던 결함은 위 항목에서 수정)
