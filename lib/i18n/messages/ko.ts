...
//
// **이 파일이 키 이름의 소스오브트루스다.** iOS 앱(`clipnote-ios`)의 문자열 카탈로그가 같은
// 이름을 쓰도록 맞춘다 — 같은 문구를 두 곳에서 다르게 부르지 않는다. 앱에만 있는 화면
// (온보딩 투어·공유 확장)은 앱 전용 네임스페이스라 여기로 가져오지 않는다.
//
...
const ko = {
   ...
   * 마크업·순서는 `app/_components/CompareBoxes.tsx` 한 곳에 있다.
   */
  compare: {
    guestTitle: "로그인 안 해도",
  ...
  /**
   * 검색·공유용 메타데이터. 화면에 직접 보이지 않지만 검색 결과와 공유 카드에 그대로 나간다.
   * OG 이미지 문구도 여기서 나온다(`/api/og` 는 넘겨받은 문자열을 그리기만 한다).
   */
  meta: {
    /** 사이트 기본 제목(페이지가 자기 제목을 주지 않을 때) */
    siteTitle: "ClipNote — URL을 깔끔한 공유 카드로",
    siteDescription:
      "URL을 입력하면 제목·설명을 자동으로 읽어와 깔끔한 공유 카드와 짧은 링크를 만들어 드려요. 네이버 카페·인스타그램도 지원합니다.",
    /** OG 이미지에 그리는 문구(짧게) */
    ogTitle: "URL을 깔끔한 공유 카드로",
